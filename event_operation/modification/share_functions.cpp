...
bool rotateDataWithTransitionMatrix(const std::unique_ptr<Data::SeismComponent>& component, const Eigen::MatrixXf& transitionMatrix) {
    ...
    for(auto& trace : component->getTraces()) {
        Eigen::Map<Eigen::RowVectorXf> b_data_vec(trace->getBuffer().get(), trace_size);
