...
RotateDataToEBasis::RotateDataToEBasis(const std::unique_ptr<Data::SeismEvent>&event, const std::map<QUuid, std::unique_ptr<Data::SeismWell>>& wells_map)
      :_event(event)
{
...
void RotateDataToEBasis::redo() {
    unsigned long i = 0;
    for (auto& component : _event->getComponents()) {
        rotateDataWithTransitionMatrix(component, _originalTransitionMatrixs[i].transpose());
        ++i;
    }
 ...
 const std::unique_ptr<SeismReceiver>& RotateDataToEBasis::findReceiver(const std::map<QUuid, std::unique_ptr<SeismWell>>& wells_map, const QUuid& receiver_uuid) {
