    ...
    _applyButton(new QPushButton("Apply Change"))
{
    ...
    connect(_applyButton, &QPushButton::clicked,
            [this]() {
        if (_radialMaxChanged || _radialMinChanged || _angularMinChanged || _angularMaxChanged) {
    ...
    connect(_radialMin, &QLineEdit::textChanged, [this] () {
        _radialMinChanged = true;
    });

    connect(_radialMax, &QLineEdit::textChanged, [this] () {
        _radialMaxChanged = true;
    });

    connect(_angularMin, &QLineEdit::textChanged, [this] () {
        _angularMinChanged = true;
    });

    connect(_angularMax, &QLineEdit::textChanged, [this] () {
        _angularMaxChanged = true;
    });
