...
PolarGraph::PolarGraph(QWidget *parent) : QFrame(parent) ,_polarChart(new QPolarChart()), _polarView(new QChartView()),
    _angularAxis(new QValueAxis()), _radialAxis(new QValueAxis)
{
...
QWidget *PolarGraph::getView() const
{
    return _allView;
}

void PolarGraph::update(const std::unique_ptr<Data::SeismEvent> &event)
{
