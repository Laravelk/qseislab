...
AnalysisWindow::AnalysisWindow(std::shared_ptr<Data::SeismEvent> &event,
                               QWidget *parent)
    : QFrame(parent), _event(event.get()),
      _hidePointsWidget(new HideWavePointsWidget()),
      _changeBorderOnPolarWidget(new ChangeBorderOnPolarWidget) {
  ...
  connect(_hidePointsWidget, &HideWavePointsWidget::updateWaveState,
          [this](auto wave, auto state) {
            switch (wave) {
            case HideWavePointsWidget::Wave::PWAVE:
              _graph->hidePWavePoints(state ==
                                      HideWavePointsWidget::State::Unchecked);
              _graph->update(_event);
              break;
            case HideWavePointsWidget::Wave::SWAVE:
              _graph->hideSWavePoints(state ==
                                      HideWavePointsWidget::State::Unchecked);
              _graph->update(_event);
              break;
            }
  ...
  connect(_changeBorderOnPolarWidget, &ChangeBorderOnPolarWidget::valueChanged,
          [this]() {
...
void AnalysisWindow::updateHodogram(const Data::SeismEvent *const event) {
...
void AnalysisWindow::setHodogramToDefault() { _hodogram->setDefault(); }

void AnalysisWindow::removePick(int numberOfReceiver,
