...
#include "data/seismproject.h" /* test for set setttings */
...
Controller::Controller(Data::SeismProject * const project, QObject *parent)
    :QObject(parent),
    _project(project),
    _model(new Model(new SegyReader(), this))
{
    auto& wells_map = project->getAllMap<Data::SeismWell>();
    auto& events_map = project->getAllMap<Data::SeismEvent>();
    auto& receivers = project->getAll<Data::SeismReceiver>();
    ...
    connect(_view.get(), &View::eventTransformClicked,
            [this, wells_map](auto oper) {
                if (!_currentEventUuid.isNull()) {
                    auto &event = _events_map[_currentEventUuid];
                    auto command = UndoCommandGetter::get(oper, QUuid(), event.get(), _project->getSettings());
                    _stacks_map[_currentEventUuid]->push(command);
                }
            });
