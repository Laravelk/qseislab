...
PolarizationAnalysisWindow::PolarizationAnalysisWindow(
  ...
  connect(event.get(), &Data::SeismEvent::changed, [this]() {
      update();
  });
...
Qt3DCore::QEntity *PolarizationAnalysisWindow::drawLine(const QVector3D& start, const QVector3D& end, const QColor& color, Qt3DCore::QEntity *_rootEntity)
{
...
void PolarizationAnalysisWindow::drawCurve(const  std::unique_ptr<Data::SeismTrace> &traceX,
                                           const std::unique_ptr<Data::SeismTrace> &traceY,
                                           const std::unique_ptr<Data::SeismTrace> &traceZ,
                                           const QColor &color, Qt3DCore::QEntity *_rootEntity,
                                           const int firstPointIndex, const int lastPointIndex, const float maxValue)
{
    auto *geometry = new Qt3DRender::QGeometry(_rootEntity);
    ...
    // position vertices (start and end)
    QByteArray bufferBytes;
    bufferBytes.resize(3 * pointsCount * sizeof(float)); // start.x, start.y, start.end + end.x, end.y, end.z / count
    float *positions = reinterpret_cast<float*>(bufferBytes.data());
...
void PolarizationAnalysisWindow::drawTraces(const std::unique_ptr<Data::SeismComponent> &component)
{
    int firstElement = 0;
    ...
    drawCurve(component->getTraces().at(0), component->getTraces().at(1), component->getTraces().at(2),
              Qt::black, _scene, firstElement, lastElement, maxValue);
}

int PolarizationAnalysisWindow::lastElementNumber(const std::unique_ptr<Data::SeismComponent> &component)
{
    int lastNumber = 0;
